...
var mapRecursive = function (expr, terminal_function = function (expr) { return expr; }) {
  var newarray = [];
  if (Array.isArray(expr)) {
    for (let n = 0; n < expr.length; n++) {
      newarray.push(mapRecursive(expr[n], terminal_function));
    }
    return newarray;
  } else {
    return terminal_function(expr);
  }
}
// CURRYING
// key dictionary key
